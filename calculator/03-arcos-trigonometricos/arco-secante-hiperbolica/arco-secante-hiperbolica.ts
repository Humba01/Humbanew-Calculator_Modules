const valoresParaRealizarArcoSecanteHiperbolica: string[] = ["30", "15"];

var total = 0;

for(let g = 0; g < valoresParaRealizarArcoSecanteHiperbolica.length; g++) {
  if(valoresParaRealizarArcoSecanteHiperbolica[g] == "") {
    total = total + 0;
  } else {
    total = total + parseFloat(valoresParaRealizarArcoSecanteHiperbolica[g]);
    console.log("Valor para realizar o arco secante hiperbólica: ", total);
  }
}
...
let resultado29 = 1 / Math.acosh(total);
console.log("Resultado do Arco Secante Hiperbólica: ", resultado29);