const valoresParaRealizarArcoCotangenteHiperbolica: string[] = ["30", "15"];

var total = 0;

for(let g = 0; g < valoresParaRealizarArcoCotangenteHiperbolica.length; g++) {
  if(valoresParaRealizarArcoCotangenteHiperbolica[g] == "") {
    total = total + 0;
  } else {
    total = total + parseFloat(valoresParaRealizarArcoCotangenteHiperbolica[g]);
    console.log("Valor para realizar o arco cotangente hiperbólica: ", total);
  }
}
...
let resultado30 = 1 / Math.atanh(total);
console.log("Resultado do Arco Cotangente Hiperbólica: ", resultado30);